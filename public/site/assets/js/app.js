...
(function ($) {
    ...
    var mobileNav = function () {
        ...
        function responsivemenu(mobile) {
          ...
          if (mobile.matches) {
            ...
            // XÃ³a táº¥t cáº£ inline style display:flex tá»« sub-menu khi chuyá»ƒn vá» mobile
            ...
            $(document).on('click', '.menu-item-inner-title', function(e) {
                // Chá»‰ xá»­ lÃ½ khi khÃ´ng pháº£i mobile menu
                if ($('#main-nav-mobile').length && $('#main-nav-mobile').hasClass('active')) {
                    return; // Äá»ƒ mobileNav xá»­ lÃ½
                }
                ...
                // Dá»«ng animation Ä‘ang cháº¡y Ä‘á»ƒ trÃ¡nh giáº­t
                $subMenuItemInner.stop(true, false);
                
                // Toggle active class - tÆ°Æ¡ng tá»± nhÆ° mobileNav xá»­ lÃ½ menu-item-inner
                $subMenuItemListing.toggleClass('active');
                
                // Sá»­ dá»¥ng slideToggle(300) vá»›i easing mÆ°á»£t mÃ  hÆ¡n
        ...
        // toggle sub-menu-item-inner trong sub-menu--listing-nav (xá»­ lÃ½ tÆ°Æ¡ng tá»± cÃ¡c submenu khÃ¡c)
        $(document).on("click", "#main-nav-mobile  .menu-item-inner-title", function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    var selectDropdown = function () {
        $('.filter-select-dropdown').each(function() {
            ...
            var updateButtonText = function() {
                ...
                // Náº¿u "all" Ä‘Æ°á»£c checked, hiá»ƒn thá»‹ "ALL [field name]" hoáº·c "all [field name]"
                if (isAllChecked) {
                    // Láº¥y name tá»« checkbox "all" hoáº·c tá»« selectName
                ...
                if (selected.length === 0) {
                    // Náº¿u khÃ´ng cÃ³ gÃ¬ Ä‘Æ°á»£c chá»n, giá»¯ nguyÃªn text hiá»‡n táº¡i hoáº·c set vá» default
    ...
    var modalPopup = function(openModalClass, bgModalClass, closeBtnClass, closeModalClass) {
        ...
        // Click close button -> Ä‘Ã³ng modal
    ...
    var ratingInput = function () {
        ...
        // Láº¥y icon tá»« data attributes hoáº·c tá»« HTML
        function getIcons($ratingInput) {
            // Æ¯u tiÃªn láº¥y tá»« data attributes
            var activeIcon = $ratingInput.data('active-icon');
            var inactiveIcon = $ratingInput.data('inactive-icon');
            
            // Náº¿u khÃ´ng cÃ³ data attributes, láº¥y tá»« icon Ä‘áº§u tiÃªn trong HTML
            if (!activeIcon || !inactiveIcon) {
                ...
                // Náº¿u khÃ´ng cÃ³ inactive icon, dÃ¹ng icon máº·c Ä‘á»‹nh tá»« HTML
                if (!inactiveIcon) {
                    inactiveIcon = defaultIconSrc;
                }
                
                // Náº¿u khÃ´ng cÃ³ active icon, suy ra tá»« inactive icon
                if (!activeIcon) {
                    ...
                    activeIcon = defaultIconSrc.replace(/star-(\d+)(-white)?\.svg/, function(match, num, white) {
                        ...
                        // Náº¿u sá»‘ >= 4, giáº£m 1; náº¿u < 4, tÄƒng 1
        ...
        function initializeRatingInputs() {
            $('.rating-input').each(function() {
                ...
                // LÆ°u icon ban Ä‘áº§u cá»§a má»—i star
                ...
                // Láº¥y icons vÃ  cache láº¡i (sáº½ tá»± Ä‘á»™ng convert cho dark mode)
    ...
    var flatAccordion = function() {
        ...
        // NgÄƒn event propagation tá»« checkbox vÃ  label cá»§a checkbox trong toggle-content
        $('.flat-accordion .toggle-content input[type="checkbox"], .flat-accordion .toggle-content label').on('click', function(e) {
            e.stopPropagation();
        });
        
        // NgÄƒn event propagation tá»« radio button (nhÆ°ng khÃ´ng pháº£i label vÃ¬ label cáº§n xá»­ lÃ½ riÃªng)
        $('.flat-accordion input[type="radio"]').on('click', function(e) {
            e.stopPropagation();
        });

        $('.flat-accordion .toggle-title').on('click', function (e) {
            // Bá» qua náº¿u click vÃ o label hoáº·c radio button
            if ($(e.target).is('label') || $(e.target).is('input[type="radio"]') || $(e.target).closest('label').length > 0) {
                return;
            }
            
            // Bá» qua náº¿u click vÃ o checkbox hoáº·c label cá»§a checkbox trong toggle-content
            ...
            // Bá» checked táº¥t cáº£ radio buttons trong flat-accordion
            $('.flat-accordion .flat-toggle').find('input[type="radio"]').prop('checked', false);
            
            // Bá» active táº¥t cáº£ flat-toggle
        ...
        // Xá»­ lÃ½ click vÃ o label Ä‘á»ƒ toggle accordion (chá»‰ label trong toggle-title, khÃ´ng pháº£i toggle-content)
    ...
    function formatAllLabel(filterType) {
        ...
        // Fallback: capitalize first letter vÃ  replace dashes
    ...
    var passwordInput = function() {
        $('.password-input').each(function () {
            ...
            $input.on('click', function (e) {
                ...
                // chá»‰ toggle khi click vÃ¹ng icon bÃªn pháº£i
    ...
    var carViewsChart = function () {
        ...
        // Custom font family
        var fontFamily = {
            labels: '"Manrope", sans-serif',      // Font cho Y vÃ  X axis labels
        ...
        var colors = {
            ...
            verticalLine: '#E7E7E7',   // Color cho vertical line bÃªn pháº£i Y-axis
        ...
        // Helper function Ä‘á»ƒ convert hex to rgba
        ...
        function resizeCanvas() {
            ...
            // Láº¥y device pixel ratio Ä‘á»ƒ trÃ¡nh hÃ¬nh trÃ²n bá»‹ mÃ©o
            ...
            // Scale context Ä‘á»ƒ Ä‘áº£m báº£o váº½ Ä‘Ãºng tá»· lá»‡
            ctx.scale(dpr, dpr);
            
            // TÃ­nh toÃ¡n láº¡i vá»›i kÃ­ch thÆ°á»›c CSS (khÃ´ng pháº£i kÃ­ch thÆ°á»›c thá»±c táº¿)
        ...
        function drawChart() {
            ...
            // Function lÃ m trÃ²n sá»‘ cho Y-axis labels
            function formatYAxisValue(value) {
                // LÃ m trÃ²n thÃ nh sá»‘ nguyÃªn
            ...
            // Draw vertical line bÃªn pháº£i Y-axis labels
            ...
            // Kiá»ƒm tra náº¿u lÃ  mobile Ä‘á»ƒ xá»­ lÃ½ labels
            var isMobile = window.matchMedia("(max-width: 767px)").matches;
            var labelStep = 1; // Máº·c Ä‘á»‹nh hiá»ƒn thá»‹ táº¥t cáº£ labels
            
            if (isMobile) {
                // TrÃªn mobile, chá»‰ hiá»ƒn thá»‹ má»—i 2 thÃ¡ng má»™t láº§n Ä‘á»ƒ trÃ¡nh overlap
                // Hoáº·c tÃ­nh toÃ¡n dá»±a trÃªn width Ä‘á»ƒ Ä‘áº£m báº£o khÃ´ng overlap
                var minLabelSpacing = 40; // Khoáº£ng cÃ¡ch tá»‘i thiá»ƒu giá»¯a cÃ¡c labels (px)
            ...
            for (var i = 0; i < months.length; i++) {
                // Chá»‰ hiá»ƒn thá»‹ labels theo step trÃªn mobile
                if (isMobile && i % labelStep !== 0 && i !== 0 && i !== months.length - 1) {
                    continue; // Bá» qua labels khÃ´ng cáº§n thiáº¿t
                }
                
                var x = chartX + monthWidth * i + monthWidth / 2;
                var y = chartY + chartHeight + 15;
                
                // TrÃªn mobile, hiá»ƒn thá»‹ sá»‘ thÃ¡ng (1, 2, 3...) thay vÃ¬ tÃªn thÃ¡ng
            ...
            var pointRadius = 6; // 12px width vÃ  height (radius * 2)
            var points = [];
            ...
            for (var i = 0; i < points.length; i++) {
                ...
                // Highlight hovered point vá»›i hiá»‡u á»©ng glow mÆ°á»£t mÃ 
                if (hoveredIndex === i && hoverAnimationProgress > 0) {
                    // Táº¡o radial gradient cho hiá»‡u á»©ng glow
                    ...
                    // Gradient tá»« mÃ u hover vá»›i opacity giáº£m dáº§n
                ...
                // Váº½ fill (bÃªn trong) - Ä‘áº£m báº£o fill Ä‘áº§y Ä‘á»§
                ctx.fillStyle = colors.dataPoint;
                ctx.beginPath();
                ctx.arc(point.x, point.y, pointRadius, 0, Math.PI * 2);
                ctx.fill();
                
                // Váº½ border chá»‰ khi focus/hover vÃ o data point
                if (hoveredIndex === i && hoverAnimationProgress > 0) { 
            ...
            if (isHovering && hoveredIndex >= 0) {
                ...
                var tooltipY = point.y - 50; // Äiá»u chá»‰nh Ä‘á»ƒ cÃ³ chá»— cho icon phÃ­a dÆ°á»›i
                var tooltipPadding = { top: 12, right: 12, bottom: 12, left: 12 }; // TÄƒng padding top vÃ  bottom Ä‘á»ƒ khÃ´ng sÃ¡t lá»
                var tooltipBorderRadius = 8;
                
                // TÃ­nh toÃ¡n kÃ­ch thÆ°á»›c tooltip dá»±a trÃªn text
                ...
                // TÃ­nh toÃ¡n height: title (16px) + gap (4px) + value (12px) + padding top (12px) + padding bottom (12px)
                ...
                // Function Ä‘á»ƒ váº½ rounded rectangle vá»›i icon tam giÃ¡c tÃ­ch há»£p
                function drawTooltipWithArrow(x, y, width, height, radius, arrowX, arrowSize) {
                    ...
                    // Bottom edge Ä‘áº¿n vá»‹ trÃ­ icon tam giÃ¡c (trÃ¡i)
                    ctx.lineTo(arrowX + arrowSize / 2, y + height);
                    // Icon tam giÃ¡c - Ä‘á»‰nh á»Ÿ bottom cá»§a tooltip
                    ctx.lineTo(arrowX, y + height + arrowSize);
                    ctx.lineTo(arrowX - arrowSize / 2, y + height);
                    // Bottom edge tiáº¿p tá»¥c
                ...
                // Draw tooltip background vá»›i border radius, box-shadow vÃ  icon tam giÃ¡c tÃ­ch há»£p
                // Box-shadow: 0px 5px 18px 5px #40485726
                // Convert #40485726: #404857 = rgb(64, 72, 87), 26 = 38/255 â‰ˆ 0.149
                var arrowSize = 6;
                var arrowX = tooltipX;
                ...
                // Draw tooltip text vá»›i padding
        ...
        // Animation function Ä‘á»ƒ lÃ m mÆ°á»£t hiá»‡u á»©ng hover
        function animateHover() {
            var targetProgress = isHovering ? 1 : 0;
            var speed = 0.15; // Tá»‘c Ä‘á»™ animation (0.1 = cháº­m, 0.3 = nhanh)
        ...
        canvas.addEventListener('mousemove', function(e) {
            ...
            // Báº¯t Ä‘áº§u animation náº¿u tráº¡ng thÃ¡i hover thay Ä‘á»•i
